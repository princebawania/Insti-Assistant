...
> **Evaluated, not just built.** On a 35-question labelled test set the system reaches **100% Hit@6 retrieval**, **~96% answer faithfulness**, and **100% out-of-scope refusal accuracy**. See [Evaluation](#evaluation) below.
...
│   ├── evaluate.py       # runs retrieval / refusal / faithfulness metrics + ablation
│   ├── eval_dataset.json # 35 labelled questions (27 in-scope, 8 out-of-scope)
...
| TOP_K | 6 | how many chunks it retrieves (see Evaluation — raised from 4 after testing) |
| MIN_SCORE | 0.30 | grounding cutoff, below this it says "I don't know" |
| LLM_MODEL | gemini-flash-latest | the Gemini model |
...
Two guards. First, before it even calls the LLM, it checks the best chunk's similarity score. If nothing clears the `MIN_SCORE` cutoff, it refuses right there, so it can't hallucinate from context it never received. Second, the system prompt tells the model to answer only from the passages it's given, cite the source number, and say it doesn't know otherwise.

Interesting finding from the evaluation below: the **LLM guard is doing most of the refusal work** — with `bge` embeddings the similarity scores for unrelated questions still sit above 0.30, so the score gate rarely fires on its own. The strict grounded-generation prompt is what actually delivers the 100% out-of-scope refusal.

## Evaluation

I didn't want to just *claim* the bot is accurate, so I built an evaluation harness (`eval/`) that measures it on a labelled set of 35 questions — 27 answerable from the corpus (3 per document) and 8 deliberately out-of-scope questions the bot should refuse.

**What it measures**
- **Retrieval** — Hit@k and MRR: does the correct source document appear in the top-k results?
- **Refusal** — end-to-end: does it correctly say "I don't know" on out-of-scope questions, and how often does it wrongly refuse an answerable one?
- **Faithfulness** — LLM-as-judge: is every claim in an answer actually supported by the retrieved context?
- **Threshold ablation** — sweeps `MIN_SCORE` to show the refusal tradeoff.

**Results**

| Metric | top-k = 4 | top-k = 6 |
| ------ | --------- | --------- |
| Retrieval Hit@k | 92.6% | **100%** |
| MRR | 0.907 | 0.921 |
| Answer faithfulness | 100% | 96.3% |
| Out-of-scope refusal accuracy | 100% | 100% |
| False-refusal rate (answerable Qs wrongly refused) | 25.9% | **18.5%** |

The evaluation surfaced a real weakness: at the original `TOP_K=4`, the bot wrongly refused about a quarter of answerable questions. The Hit@k sweep showed retrieval reached 100% at k=6, so I raised `TOP_K` to 6 — which cut the false-refusal rate from 25.9% to 18.5% and lifted retrieval to 100%, at only a small faithfulness cost. That's why the default is now 6.
...
# full run incl. faithfulness + end-to-end refusal (needs GEMINI_API_KEY)
python eval/evaluate.py --mode full --rpm 15
```

Results are printed and written to `eval/eval_results.json` and `eval/eval_report.md`.
